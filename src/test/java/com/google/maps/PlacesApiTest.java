...
public class PlacesApiTest {
	...
	public void testAutocompletePredictionStructuredFormatting() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(autocompletePredictionStructuredFormatting)) {
			final AutocompletePrediction[] predictions = PlacesApi.placeAutocomplete(sc.context, "1").await();
	...
	public void testRadarSearchRequest() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("{\"status\" : \"OK\"}")) {
			LatLng location = new LatLng(10, 20);
			PlacesApi.radarSearchQuery(sc.context, location, 5000).keyword("keyword").language("en")
					.minPrice(PriceLevel.INEXPENSIVE).maxPrice(PriceLevel.EXPENSIVE).name("name").openNow(true)
					.type(PlaceType.AIRPORT).await();
	...
	public void testPlaceAutocompleteRequest() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("{\"status\" : \"OK\"}")) {
			LatLng location = new LatLng(10, 20);
			PlacesApi.placeAutocomplete(sc.context, "Sydney Town Hall").offset(4).location(location).radius(5000)
					.types(PlaceAutocompleteType.ESTABLISHMENT).components(ComponentFilter.country("AU")).await();
	...
	public void testRadarSearchRequestByName() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(placesApiRadarSearchRequestByName)) {
			PlacesSearchResponse response = PlacesApi.radarSearchQuery(sc.context, SYDNEY, 10000)
					.name("Sydney Town Hall").await();
	...
	public void testRadarSearchRequestByType() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(placesApiRadarSearchRequestByType)) {
			PlacesSearchResponse response = PlacesApi.radarSearchQuery(sc.context, SYDNEY, 10000).type(PlaceType.BAR)
					.await();
	...
	public void testPlaceAutocomplete() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(placesApiPlaceAutocomplete)) {
			AutocompletePrediction[] predictions = PlacesApi.placeAutocomplete(sc.context, "Sydney Town Ha").await();
	...
	public void testPlaceAutocompleteWithStrictBounds() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(placesApiPlaceAutocomplete)) {
			PlacesApi.placeAutocomplete(sc.context, "Amoeba").types(PlaceAutocompleteType.ESTABLISHMENT)
