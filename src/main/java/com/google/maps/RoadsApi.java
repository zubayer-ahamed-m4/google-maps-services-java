...
/**
 * The Google Maps Roads API identifies the roads a vehicle was traveling along
 * and provides additional metadata about those roads, such as speed limits.
 *
 * <p>
 * See also:
 * <a href="https://developers.google.com/maps/documentation/roads">Roads API
 * documentation</a>.
 */
public class RoadsApi {
	static final String API_BASE_URL = "https://roads.googleapis.com";

	static final ApiConfig SNAP_TO_ROADS_API_CONFIG = new ApiConfig("/v1/snapToRoads").hostName(API_BASE_URL)
			.supportsClientId(false).fieldNamingPolicy(FieldNamingPolicy.IDENTITY);

	static final ApiConfig SPEEDS_API_CONFIG = new ApiConfig("/v1/speedLimits").hostName(API_BASE_URL)
			.supportsClientId(false).fieldNamingPolicy(FieldNamingPolicy.IDENTITY);

	static final ApiConfig NEAREST_ROADS_API_CONFIG = new ApiConfig("/v1/nearestRoads").hostName(API_BASE_URL)
			.supportsClientId(false).fieldNamingPolicy(FieldNamingPolicy.IDENTITY);
	...
	/**
	 * Takes up to 100 GPS points collected along a route, and returns a similar set
	 * of data with the points snapped to the most likely roads the vehicle was
	 * traveling along.
	 *
	 * @param context
	 *            The {@link GeoApiContext} to make requests through.
	 * @param path
	 *            The collected GPS points as a path.
	 ...
	 * Takes up to 100 GPS points collected along a route, and returns a similar set
	 * of data with the points snapped to the most likely roads the vehicle was
	 * traveling along. Additionally, you can request that the points be
	 * interpolated, resulting in a path that smoothly follows the geometry of the
	 * road.
	 *
	 * @param context
	 *            The {@link GeoApiContext} to make requests through.
	 * @param interpolate
	 *            Whether to interpolate a path to include all points forming the
	 *            full road-geometry. When true, additional interpolated points will
	 *            also be returned, resulting in a path that smoothly follows the
	 *            geometry of the road, even around corners and through tunnels.
	 * @param path
	 *            The path to be snapped.
	 * @return Returns the snapped points as a {@link PendingResult}.
	 */
	public static PendingResult<SnappedPoint[]> snapToRoads(GeoApiContext context, boolean interpolate,
			LatLng... path) {
		return context.get(SNAP_TO_ROADS_API_CONFIG, RoadsResponse.class, "path", join('|', path), "interpolate",
				String.valueOf(interpolate));
	}

	/**
	 * Returns the posted speed limit for given road segments. The provided LatLngs
	 * will first be snapped to the most likely roads the vehicle was traveling
	 * along.
	 *
	 * <p>
	 * Note: The accuracy of speed limit data returned by the Google Maps Roads API
	 * cannot be guaranteed. Speed limit data provided is not real-time, and may be
	 * estimated, inaccurate, incomplete, and/or outdated. Inaccuracies in our data
	 * may be reported through <a href=
	 ...
	 *            {@link #snapToRoads(GeoApiContext, com.google.maps.model.LatLng...)}
	 *            method. You can pass up to 100 placeIds with each request.
	 * @return Returns the speed limits as a {@link PendingResult}.
	 */
	...
	/**
	 * Returns the result of snapping the provided points to roads and retrieving
	 * the speed limits.
	 *
	 * @param context
	 *            The {@link GeoApiContext} to make requests through.
	 * @param path
	 *            The collected GPS points as a path.
	 * @return Returns the snapped points and speed limits as a
	 *         {@link PendingResult}.
	 */
	 ...
	 *            The {@link GeoApiContext} to make requests through.
	 * @param points
	 *            The sequence of points to be aligned to nearest roads
	 * @return Returns the snapped points as a {@link PendingResult}.
	 */
