 ...
 *
 * <p>
 * {@code T} is the class of the result, {@code A} is the actual base class of
 * this abstract class, and R is the type of the request.
 */
abstract class PendingResultBase<T, A extends PendingResultBase<T, A, R>, R extends ApiResponse<T>>
	...
	private PendingResult<T> makeRequest() {
		...
		default:
			throw new IllegalStateException(String.format("Unexpected request method '%s'", config.requestVerb));
		}
	}
	...
	/**
	 * The language in which to return results. Note that we often update supported
	 * languages so this list may not be exhaustive.
	 *
	 * @param language
	 *            The language code, e.g. "en-AU" or "es".
	 * @see <a href="https://developers.google.com/maps/faq#languagesupport">List of
	 *      supported domain languages</a>
	...
	/**
	 * A channel to pass with the request. channel is used by Google Maps API for
	 * Work users to be able to track usage across different applications with the
	 * same clientID. See <a href=
	 * "https://developers.google.com/maps/documentation/business/clientside/quota">Premium
	 * Plan Usage Rates and Limits</a>.
	 *
	 * @param channel
