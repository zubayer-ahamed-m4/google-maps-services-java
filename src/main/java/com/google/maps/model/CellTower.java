 ...
 *
 * <p>
 * The Geolocation API request body's cellTowers array contains zero or more
 * cell tower objects.
 *
 * <p>
 * Please see <a href=
 * "https://developers.google.com/maps/documentation/geolocation/intro#cell_tower_object">Cell
...
public class CellTower implements Serializable {
	...
	private CellTower(Integer _cellId, Integer _locationAreaCode, Integer _mobileCountryCode,
			Integer _mobileNetworkCode, Integer _age, Integer _signalStrength, Integer _timingAdvance) {
		this.cellId = _cellId;
	 ...
	 * UTRAN/GERAN Cell Identity (UC-Id), which is a 32-bit value concatenating the
	 * Radio Network Controller (RNC) and Cell ID. Specifying only the 16-bit Cell
	 * ID value in WCDMA networks may return inaccurate results.
	 */
	public Integer cellId = null;
	/**
	 * The Location Area Code (LAC) for GSM and WCDMAnetworks or The Network ID
	 * (NID) for CDMA networks (required).
	 */
	...
	/**
	 * The cell tower's Mobile Network Code (required). This is the MNC for GSM and
	 * WCDMA; CDMA uses the System ID (SID).
	 */
	public Integer mobileNetworkCode = null;
	/*
	 * The following optional fields are not currently used, but may be included if
	...
	public static class CellTowerBuilder {
		...
		public CellTower createCellTower() {
			return new CellTower(_cellId, _locationAreaCode, _mobileCountryCode, _mobileNetworkCode, _age,
					_signalStrength, _timingAdvance);
		}
